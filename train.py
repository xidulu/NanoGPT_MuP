...
from nano_gpt import GPTConfig, GPT


# -----------------------------------------------------------------------------
# default config values designed to train a gpt2 (124M) on OpenWebText
# I/O
out_dir = '/home/xwang3_umass_edu/gdata/why-weight-decay/output'
eval_interval = 2000
log_interval = 1
eval_examples = 1000
...
learnable_ln = False # Original: True
base_n_embd = 48 # for MuP
init_scale = 0.02
weight_tying = True
...
dataset_ratio = 1.0 # Use subset
base_shape_file = None # For MuP
decoupled_wd = False # For MuAdamW
# -----------------------------------------------------------------------------
...
data_dir = '/home/xwang3_umass_edu/gdata/nanoGPT/data/openwebtext'
train_data = np.memmap(os.path.join(data_dir, 'train.bin'), dtype=np.uint16, mode='r')
if dataset_ratio != 1:
    N = len(train_data)
    NUM_CHUNK = 8
    chunk_indices = np.array_split(np.arange(N), NUM_CHUNK) # First split all tokens into 8 chunk
    subset_indices = np.concatenate([
        chunk_indices[i]
        for i in np.random.choice(NUM_CHUNK, int(NUM_CHUNK * dataset_ratio), replace=False)
    ])
...
model_args = dict(n_layer=n_layer, n_head=n_head, n_embd=n_embd, block_size=block_size,
                  ...
                  init_scale=init_scale, weight_tying=weight_tying) # start with model_args from command line
...
# For MuP, set assert as False, otherwise it will complain not using MuReadout, 
# which is not happy with torch.compile
set_base_shapes(model, base_shape_file, rescale_params=True, do_assert=False) 
model.to(device)
...
# optimizer
optimizer = model.configure_optimizers(opt_type, weight_decay, learning_rate, (beta1, beta2),
                                       device_type, wd_substrings_include, dampening,
...
def get_lr(it):
    ...
    if not use_cosine_decay:
        return max(
            min_lr,
            learning_rate * ((it + 1 - warmup_iters) ** (-0.5))
        )
    else:
